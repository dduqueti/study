...
# exactly  steps. For every step he took, he noted if it was an uphill, U, or a
# downhill, D step. Gary's hikes start and end at sea level and each step up
...
# Then he climbs out an up onto a mountain  units high. Finally, he returns
...
def countingValleys(n, s)
  ...
  s.split('').each do |step|
    level = step.eql?('D') ? level - 1 : level + 1

    if step.eql?('U') && level == 0
        valleys_counter += 1
    end
  end
